...
fn print_line_result(is_checksum_correct: bool, opts: ChecksumOptions, filename: &str) {
        ...
        chosen_regex.captures(os_str_as_bytes(line).expect("UTF-8 decoding failure"))
    {
        *properly_formatted = true;
        ...
        let expected_checksum = get_expected_checksum(&filename_lossy, checksum)
            .map_err(|_| LineCheckError::ImproperlyFormatted(false))?;
        ...
        return Err(LineCheckError::ImproperlyFormatted(false));
    }
}
