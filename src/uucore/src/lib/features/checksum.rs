...
impl AlgoKind {
    ...
    pub fn to_tag(self) -> &'static str {
        ...
        match self {
            ...
            // These algorithms are not useable with the tagged output.
            Sysv | Bsd | Crc | Crc32b => "UNTAGGABLE",
        }
    }
...
pub fn detect_algo(algo: AlgoKind, length: Option<usize>) -> UResult<HashAlgorithm> {
    match algo {
        ...
        AlgoKind::Shake128 => {
            let bits = length.ok_or(ChecksumError::LengthRequired(algo.to_tag().to_string()))?;
            Ok(HashAlgorithm {
        ...
        AlgoKind::Sha2 => {
            let bits = validate_sha2_sha3_length(algo.to_lowercase(), length)?;
            create_sha2(bits)
        }
        AlgoKind::Sha3 => {
            let bits = validate_sha2_sha3_length(algo.to_lowercase(), length)?;
...
pub fn sanitize_sha2_sha3_length_str(algo_kind: AlgoKind, length: &str) -> UResult<usize> {
    ...
    let len = match length.parse::<usize>() {
        ...
        Err(e) if *e.kind() == IntErrorKind::PosOverflow => {
            show_error!("{}", ChecksumError::InvalidLength(length.into()));
            return Err(ChecksumError::InvalidLengthForSha(algo_kind.to_tag().into()).into());
        }
    ...
    if [224, 256, 384, 512].contains(&len) {
        ...
        show_error!("{}", ChecksumError::InvalidLength(length.into()));
        Err(ChecksumError::InvalidLengthForSha(algo_kind.to_tag().into()).into())
    }
}
