...
use std::path::Path;
use uucore::checksum::{
    calculate_blake2b_length, digest_reader, perform_checksum_validation, ChecksumAlgoBuilder, ChecksumError, ALGORITHM_OPTIONS_BLAKE2B, ALGORITHM_OPTIONS_BSD, ALGORITHM_OPTIONS_CRC, ALGORITHM_OPTIONS_SYSV, SUPPORTED_ALGORITHMS
};
use uucore::{
    encoding,
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let mut cab = ChecksumAlgoBuilder::new(None);
    cab.algo_name(Some(algo_name.to_string())).algo_length(length);
    let algo = cab.try_build().unwrap();
